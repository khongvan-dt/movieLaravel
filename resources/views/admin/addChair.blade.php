        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="container-fluid px-4">
                    <h1 class="mt-4">Thêm Ghế</h1>
                    <div class="card mb-4">
                        <div class="card-body">
                            <form method="post" action="{{ route('saveChair') }}" enctype="multipart/form-data">
                                @csrf
                                <table>
                                    <thead>
                                        <tr>
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                @else
                                                    <select class="form-control" id="sel1" name="rapChieu">
                                                        @foreach ($ListRapChieu as $rapChieu)
                                                            <option value="{{ $rapChieu->rap_chieu_id }}">
                                                                {{ $rapChieu->Name_rap }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                                        ...
                                        <tr>
                                            <td>
                                                <input class="form-control" type="text" name='soGhe'
                                                    placeholder="Nhập Số Ghế">
                                            </td>
                                        </tr>

                                        <tr>
                                            <td>
                                                <input class="form-control" type="text" name='loaiGhe'
                                                    placeholder="Nhập Loại Ghế">
                                            </td>
                                        </tr>
                                        <tr>
                                            <td>
                                                <input class="form-control" type="text"name='price'
                                                    placeholder="Giá Ghế">
                                            </td>
                                        </tr>
                                        <tr>
                            ...
                            <div class="d-flex flex-wrap">
                                @if ($innerJoin->isEmpty())
                                    <div class="alert alert-info">không có thông tin.</div>
                                @else
                                    @foreach ($innerJoin as $index => $item)
                                        <div style="margin: 10px; padding: 10px; border: 1px solid #ebebeb;">
                                            <p>Tên rạp: {{ $item->Name_rap }}</p>
                                            <p>Số ghế: {{ $item->So_ghe }}</p>
                                            <p>Loại ghế: {{ $item->Looi_ghe }}</p>
                                            <p>Giá: {{ number_format($item->price) }}</p>
                                            <div style="display:flex ; gap:10px">
                                                <div>
                                                    <a class="btn btn-danger" href="{{ route('edit', ['ChairId' => $item->ghe_ngoi_id]) }}">Sửa</a>
                                                </div>
                                                <div>
                                                    <form method="post"
                                                        action="{{ route('deleteChair', ['ChairId' => $item->ghe_ngoi_id]) }}">
                                                        @csrf
                                                        @method('DELETE')
